...
func (o *Orchestrator) ReceiveMessage(ctx context.Context, message msg.Message) error {
	...
	if err != nil {
		...
		logger.Error("error decoding reply message payload", edatlog.Error(err))
		return nil
	}
	...
	if err != nil {
		logger.Error("failed to locate saga instance data", edatlog.Error(err))
		return nil
	}
	...
	if err != nil {
		logger.Error("saga reply handler returned an error", edatlog.Error(err))
		return err
	}
...
func (o *Orchestrator) replyMessageInfo(message msg.Message) (string, string, string, error) {
	...
	if err != nil {
		o.logger.Error("error reading reply name", edatlog.Error(err))
		return "", "", "", err
	}

	sagaID, err = message.Headers().GetRequired(MessageReplySagaID)
	if err != nil {
		o.logger.Error("error reading saga id", edatlog.Error(err))
		return "", "", "", err
	}
...
func (o *Orchestrator) processResults(ctx context.Context, instance *Instance, results *stepResults) error {
	...
	for {
		if results.failure != nil {
			...
			if err != nil {
				logger.Error("error handling local failure result", edatlog.Error(err))
				return err
			}
		} else {
			for _, command := range results.commands {
				err = o.publisher.PublisheCommand(ctx, o.definition.ReplyChannel(), command, WithSagaInfo(instance))
				if err != nil {
					return err
				}
			}
...
func (o *Orchestrator) handleReply(ctx context.Context, stepCtx stepContext, sagaData core.SagaData, message msg.Reply) (*stepResults, error) {
	...
	logger := o.logger.Sub(
		edatlog.String("SagaName", o.definition.SagaName()),
		edatlog.String("SagaID", message.Headers().Get(MessageReplySagaID)),
		edatlog.String("ReplyName", replyName),
	)
	...
	if err != nil {
		logger.Error("error reading reply outcome", edatlog.Error(err))
		return nil, err
	}

	logger.Trace("reply outcome", edatlog.String("Outcome", outcome))
	...
	case stepCtx.compensating:
		...
		logger.Error("received a failure outcome while compensating", edatlog.Int("Step", stepCtx.step))
