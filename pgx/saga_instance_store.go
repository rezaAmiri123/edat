...
func (s *SagaInstanceStore) Find(ctx context.Context, sagaName, sagaID string) (*saga.Instance, error) {
	...
	return saga.NewSagaInstance(dataName, sagaID, sagaData, currentStep, endState, compensating), nil
}
...
func (s *SagaInstanceStore) Save(ctx context.Context, sagaInstance *saga.Instance) error {
	...
	_, err = s.client.Exec(ctx, fmt.Sprintf(saveSagaInstanceSQL, s.tableName),
		sagaInstance.SagaName(),
		sagaInstance.SagaID(),
		sagaInstance.SagaData().SagaDataName(),
		data,
		sagaInstance.CurrentStep(),
		sagaInstance.EndState(),
		sagaInstance.Compensating(),
	)
...
func (s *SagaInstanceStore) Update(ctx context.Context, sagaInstance *saga.Instance) error {
	...
	_, err = s.client.Exec(ctx, fmt.Sprintf(updateSagaInstanceSQL, s.tableName),
		data,
		sagaInstance.CurrentStep(),
		sagaInstance.EndState(),
		sagaInstance.Compensating(),
