...
func (b Backoff) nextInterval(latsInterval time.Duration) time.Duration {
	// Either there isn't any delay or there is not growth
	if b.initialInterval == 0 || latsInterval == b.maxInterval {
		return latsInterval
	}

	next := float64(latsInterval) * b.multiplier
